...
\subsection{Online similarity learning}
For our problem of online similarity learning we adopt LinUCB \cite{linucb} and  NeuralUCB \cite{neuralucb}.
For this problem both of our proposed algorithms are extensions of the aforementioned algorithms. We refer

\subsubsection{OnSim-LinUCB}
With respect to LinUCB \cite{linucb} we propose a reward as described in (\ref{problem-statement:online-similarity-learning}), and context structure which assumes that $\phi(\mathbf{x}, \mathbf{y}) = \mathbf{x}^\top \mathbf{A} \mathbf{y}$.
...
which we is linear in $\mathbf{A}$.
